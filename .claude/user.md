...
- **ドリフト検知・`pkill` 失敗の通知先**：専用のアラート基盤は新設しない。cron_batch.md と同じ仕組み（`/var/log/owl/` への記録 + ジョブが非ゼロ終了することで OpenBSD cron の標準動作である root 宛メールに乗せる）に統合する。少数運用ではログ監視と cron メールの目視で十分であり、新規の通知経路は過剰投資と判断する。
