...
- 索引ファイルのディスク使用量上限は明文化しない（newsyslog の30世代保持と同じ期間だけ追従するため、原本ログの保持期間が事実上の上限になる）。ただし起動時（`main.go` の `healthCheckIndexSize`）に索引サイズ（`PRAGMA page_count * page_size`）が原本ログ合計サイズ（`/var/log/audit/remote.log*` の総バイト数）の一定倍数（既定 3倍、§1.1）を超えていないかを検査し、超えていれば標準ログへ `WARNING` を出力する（プロセスは継続する——ヘルスチェックの失敗で起動自体を止める設計にはしない。原本ログが0バイトの初回起動時は比較自体をスキップする）。想定外の索引肥大（パースバグ等）を早期検知する。
...
| Z-score / Shewhart 管理図（3σ規則） | Shewhart (1931) の統計的工程管理 | 単位時間あたりの `AuthSuccess`/`PrivilegeEscalationAttempt`/`AuthFailure` 件数 | 直近の値が移動平均から3σを超えて離れている（点異常） |
| Western Electric Rules（2-of-3 ルール） | Western Electric Company (1956) | 同上、直近3点の z-score 履歴 | 直近3点中2点以上が2σを同じ側で超える持続的なズレ — 単発では3σに届かないが、3σ規則だけでは見逃す中程度の異常を捉える（メソッド名 `wer-2of3`） |
...
- 各手法はカテゴリキー(`(SourceHost, Category)` の組)ごとに独立した状態（平均・分散・EWMA値・CUSUM累積値・頻度表・WER用の直近3点の z-score 履歴）を持ち、`AuditEvent` 1件ごと（頻度表）または取り込みサイクル1回ごと（Welford/EWMA/CUSUM/WER、§4.1①の「ストリーミング取り込み」の単位と一致させる）に定数時間で更新する。履歴イベントの再走査は不要（オンラインアルゴリズムの定義そのもの）。
- 状態は `/var/db/fohlen/state/stats.json` に定期保存し（取り込みサイクルごと、`main.go`）、`fohlen` 再起動時に学習をやり直さない。WER 用の z-score 履歴もこのスナップショットに含める。
- 各手法の閾値（σ係数、CUSUM の許容偏差 `k` と検出閾値 `h`、EWMA の減衰係数 `λ`、エントロピーの「サプライズ量」閾値）は `/etc/owlv/fohlen.toml`（`internal/config`）で調整可能。初期値は実機収集後に経験的に校正する（§10 残課題）。WER の窓サイズ(3点)・必要ヒット数(2点)・σ閾値(2.0)は固定値とする（手法そのものの定義であり、運用パラメータではないため設定対象に含めない）。
- 検知結果は `/var/db/fohlen/index.sqlite3` の `detections` テーブルへ永続化する（`internal/index`）。ダッシュボードの「直近の逸脱検知イベント一覧」(§4.4)はこのテーブルを参照するため、プロセス再起動後も履歴が失われない。
...
- 提供画面（読み取り専用、書き込み操作は一切提供しない）:
  - **検索**: §4.2 のキーワード・時間範囲（`<input type="datetime-local">` 2個）・ホスト・カテゴリフィルタ。結果は固定50件ずつページネーションし（次へ/前へ）、一度に全件をメモリへ展開しない。
  - **タイムライン/ダッシュボード**: §4.3 の各カテゴリの現在の統計量（移動平均・EWMA・CUSUM累積値）と、`detections` テーブルから読み出す直近の逸脱検知イベント一覧（再起動後も保持される、§4.3）。`AuditEvent.Category` 別に色分けし、`Unclassified` の出現率も常時表示する(§4.1の「ルールの限界に対する保険」を運用者が目視できるようにする)。
...
- **検索UIの時間範囲フィルタ・ページネーション**: §4.2/§4.4 で要求していたが UI 側に入力欄・次へ/前へ操作が存在しなかった。`internal/web` の `search.gohtml`・`parseSearchFilter` に実装し、固定50件単位でページ送りする。
- **索引サイズのヘルスチェック**: §4.2 が要求する起動時チェック（`Index.SizeBytes` と原本ログ合計サイズの比較）はメソッドのみ存在し呼び出されていなかった。`main.go` の `healthCheckIndexSize` から起動時に呼び出すよう結線した。
- **逸脱検知履歴の永続化**: ダッシュボードの「直近の逸脱検知」がプロセス内メモリのリングバッファのみで、再起動すると履歴が消えていた。`internal/index` に `detections` テーブルを追加し、検知のたびに永続化、ダッシュボードはそこから読み出す方式に変更した。
- **Western Electric Rules の実装**: §4.3 の手法表は学術的根拠として Western Electric Rules を引用していたが、実装は単発3σ規則のみだった。「直近3点中2点以上が2σ超」(2-of-3) ルールを追加し、3σに届かない中程度の持続的シフトも検知できるようにした。
