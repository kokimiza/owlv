...
_ssh_ap "sed \
        -e 's/^OWL_DB_USER=.*/OWL_DB_USER=${DB_APP_USER}/' \
        -e 's#^OWL_DB_PASS=.*#OWL_DB_PASS=${APP_PASS}#' \
        -e 's/^OWL_DB_NAME=.*/OWL_DB_NAME=${DB_NAME}/' \
        /etc/owl/db.env.template > /etc/owl/db.env
     chmod 640 /etc/owl/db.env" ||
	_die "db.env の生成に失敗しました"
_ok "/etc/owl/db.env 生成完了 (chmod 640)"
