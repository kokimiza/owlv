...
#   STEP 2: WAN 正規化 (default route 再投入。WAN_IF 自体の復元は不要)
#   STEP 3: vether/bridge インターフェース / hostname.* / login.conf datasize
#   STEP 4: IP フォワーディング / PF
#   STEP 5: VM ディスクイメージ (--wipe 時のみ)
#   STEP 6: SSH 鍵 (プロビジョニング鍵・バックアップ鍵) / autoinstall サーバー残骸
#   STEP 9: rc.conf.local / 改ざん検知ベースライン

set -eu

[ "$(id -u)" -eq 0 ] || {
	echo "root (doas) で実行してください" >&2
	exit 1
}
[ "$(uname)" = "OpenBSD" ] || {
	echo "OpenBSD でのみ実行可能" >&2
	exit 1
}
...
# rcctl で落ちきらないゾンビプロセスを SIGKILL (STEP 3 と同じ手順)
...
# ── STEP 4: PF (NAT) ────────────────────────────────────────
_log "STEP 4: IP フォワーディング / PF をリセット"
...
# ── STEP 6: autoinstall サーバー残骸 ─────────────────────────
_log "STEP 6: autoinstall サーバー残骸を削除"
...
# /etc/vm.conf は STEP 3 (最小構成) → STEP 7 (本番構成) で書き込まれる
...
# git-jump アカウント (owl-git-jump グループに属する全ユーザー) を削除
for _user in $(getent group owl-git-jump 2>/dev/null | awk -F: '{print $4}' | tr ',' ' '); do
	[ -n "$_user" ] || continue
	userdel -r "$_user" 2>/dev/null && _ok "ユーザー ${_user} 削除 (git-jump)" ||
		_warn "${_user} ユーザーの削除に失敗。手動で: userdel -r ${_user}"
done
groupdel owl-git-jump 2>/dev/null && _ok "グループ owl-git-jump 削除" || _skip "owl-git-jump グループ (存在しない)"
rm -f /etc/ssh/sshd_config.d/git-jump.conf && _ok "削除: /etc/ssh/sshd_config.d/git-jump.conf" ||
	_skip "/etc/ssh/sshd_config.d/git-jump.conf"
...
# login.conf の daemon datasize=infinity を削除 (STEP 3 が追加)
...
# ── STEP 9: rc.conf.local ────────────────────────────────────
_log "STEP 9: rc.conf.local をリセット"
